...
BUILDAH_FORMAT=docker podman buildx build -v /etc/pki/entitlement:/etc/pki/entitlement:ro -t quay.io/zenithtecnologia/zerotier-docker:${zt_version} --build-arg zt_version=${zt_version} .
BUILDAH_FORMAT=docker podman buildx build -v /etc/pki/entitlement:/etc/pki/entitlement:ro -t quay.io/zenithtecnologia/zerotier-docker:${zt_version}-debian --build-arg zt_version=${zt_version} -f Dockerfile.debian .
